...
- 예시: "안녕. ||| 반가워."
  ...
  if (stage === 1) {
    ...
    if (branch === "Stop") {
      ...
      `;
      quickReplyGuide = `끔찍한 신체적 고통에 대한 반응입니다. 대답 후보 3개를 "당황스럽게 느껴질 것 같아", "상상이 되지 않아", "${who}이 속상해 할 것 같아" 처럼 구성하세요.`;
    }
  }
  else if (stage === 4) {
    if (branch === "Stop") {
    ...
    if (branch === "Stop") {
      ...
      `;
      quickReplyGuide = `현실적인 고민에 대한 반응입니다. 후보 3개를 "당장 병원비는 어떡하지?", "가족 생활비가 제일 걱정돼", "앞으로 다시 현장에서 일하지 못하게 될 까봐 걱정돼" 같이 작성하세요.`;
    }
  }
  else { 
    if (branch === "Stop") {
